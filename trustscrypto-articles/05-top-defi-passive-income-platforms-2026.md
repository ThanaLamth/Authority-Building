...
- EigenLayer AVS-specific slashing conditions were not reviewed for each live AVS — the slashing risk is described categorically, not mapped to specific AVS parameters
- Ethena's USDe funding rate history during prior negative rate periods (when yield went to zero or negative) was not modeled with specific historical dates and duration
- Aave E-Mode LTV interactions with specific collateral pair combinations were not independently calculated — the general E-Mode mechanic is described but edge cases are not mapped
- Pendle yield token markets for specific assets (stETH, USDE) were not analyzed — the PT/YT framework is explained but individual market selection is not covered

If you deposit in a DeFi protocol for yield and found a risk or workflow gap this article didn't address, that is the most useful feedback.
