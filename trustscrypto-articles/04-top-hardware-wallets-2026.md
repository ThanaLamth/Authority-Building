...
> **Data freshness:** Device pricing, firmware versions, and companion app compatibility in this article reflect July 2026 data. Hardware wallet features and security models change with firmware updates. Verify current firmware version at each manufacturer's site before purchasing. The signing model and open-source status comparisons are structural and more stable.
## Ranking scorecard
...
- End-to-end firmware verification was not completed for any device — security architecture is reviewed from documentation and published audit reports, not from testing a physical unit
- Coldcard Q PSBT compatibility with specific wallet coordinators (Sparrow, Specter, BlueWallet) was not tested across each combination
- Battery life, physical durability, and failure rates under normal use conditions are not covered — this article evaluates security architecture and software features, not hardware longevity
- Ledger Recover opt-in mechanics and associated risk profile are not analyzed in depth — the closed-source Secure Element firmware limitation is noted but the Recover service debate is a separate question

If you tested a device in this list and found a firmware behavior or compatibility gap not reflected here, that is exactly the kind of firsthand data this article is missing.
